...
namespace Trigger2to3
{
    public class T23_ModuleEditorBase : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                body_base.groupID = EditorGUILayout.IntField("ID группы", body_base.groupID);
            }
        ...
        public static bool GuideJoinMaster(T23_Master master, T23_ModuleBase body, int gid, int category)
        {
            if (master == null)
            {
                EditorGUILayout.HelpBox("Модуль не подключён к Master. Работает и без него, но можно подключить кнопкой ниже.", MessageType.Info);
                if (GUILayout.Button("Подключить к Master"))
                {
