...
namespace Trigger2to3
{
    ...
    internal class T23_MasterEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            serializedObject.FindProperty("reliable").boolValue = EditorGUILayout.Toggle("Синхронизация Manual", master.reliable);
            ...
            {
                EditorGUILayout.HelpBox("VRC_ObjectSync присутствует — Synchronize Method принудительно установлен в Continuous.", MessageType.Info);
            }

            EditorGUILayout.BeginHorizontal();
            Color oldBackgroundColor = GUI.backgroundColor;
            if (_copyTarget != null && _copyTarget == master) { GUI.backgroundColor = Color.green; }
            if (GUILayout.Button("Выбрать источник копирования"))
            {
            ...
            if (GUILayout.Button("Копировать модули"))
            {
                CopyModules();
            }
            EditorGUI.EndDisabledGroup();
            ...
            if (master.shouldMoveComponents)
            {
                ...
                if (prefab == null)
                {
                    ...
                    {
                        msg = "Не удалось правильно упорядочить компоненты.";
                    }
                }
                else
                {
                    msg = "Компоненты внутри Prefab — упорядочить невозможно.";
                }
                if (msg != "")
                {
                    EditorGUILayout.BeginHorizontal();
