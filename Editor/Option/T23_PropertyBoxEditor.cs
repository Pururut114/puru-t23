...
namespace Trigger2to3
{
    ...
    internal class T23_PropertyBoxEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            serializedObject.FindProperty("valueType").intValue = (int)(ValueType)EditorGUILayout.EnumPopup("Тип значения", (ValueType)body.valueType);
            if (body.valueType == 0)
            {
                serializedObject.FindProperty("value_b").boolValue = EditorGUILayout.Toggle("Значение", body.value_b);
            }
            else if (body.valueType == 1)
            {
                serializedObject.FindProperty("value_i").intValue = EditorGUILayout.IntField("Значение", body.value_i);
            }
            else if (body.valueType == 2)
            {
                serializedObject.FindProperty("value_f").floatValue = EditorGUILayout.FloatField("Значение", body.value_f);
            }
            else if (body.valueType == 3)
            {
                serializedObject.FindProperty("value_v3").vector3Value = EditorGUILayout.Vector3Field("Значение", body.value_v3);
            }
            else if (body.valueType == 4)
            {
                serializedObject.FindProperty("value_s").stringValue = EditorGUILayout.TextField("Значение", body.value_s);
            }

            UdonSharpGUI.DrawUILine(Color.gray);

            serializedObject.FindProperty("trackType").intValue = (int)(TrackType)EditorGUILayout.EnumPopup("Тип отслеживания", (TrackType)body.trackType);
            if (body.trackType != 0)
            {
                List<string> spotList = new List<string>();
                if (body.trackType == 1)
                {
                    serializedObject.FindProperty("targetPlayer").intValue = (int)(TargetPlayer)EditorGUILayout.EnumPopup("Игрок", (TargetPlayer)body.targetPlayer);
                    ...
                    if (body.targetPlayer == 2)
                    {
                        ...
                        var triggerIndex = EditorGUILayout.Popup("Триггер", triggerComponentList.IndexOf(body.targetTrigger), triggerList.ToArray());
                ...
                if (body.trackType == 4)
                {
                    ...
                    if (body.targetObject != null)
                    {
                        if (body.targetComponent == null)
                        {
                            EditorGUILayout.HelpBox($"Нет UI-компонентов подходящего типа ({(ValueType)body.valueType}) на объекте.", MessageType.Error);
                        }
                        else
                        {
                ...
                {
                    prop = serializedObject.FindProperty("targetComponent");
                    prop.objectReferenceValue = EditorGUILayout.ObjectField("Аниматор", prop.objectReferenceValue, typeof(Animator), true);
                ...
                if (body.trackType == 6)
                {
                    EditorGUILayout.HelpBox("Тип 'Контроллер' не поддерживается в Udon (Input.GetAxis недоступен). Выберите другой тип отслеживания.", MessageType.Error);
                }

                if (spotList.Count > 0)
                {
                    var spotIndex = EditorGUILayout.Popup("Свойство", spotList.IndexOf(body.spot), spotList.ToArray());
                ...
                if (body.valueType == 2)
                {
                    ...
                    if (changeable.Contains(body.spot))
                    {
                        ...
                        var detailIndex = EditorGUILayout.Popup("Компонент вектора", detailList.IndexOf(body.spotDetail), SpotDetail_v3_f);
                ...
                if (body.valueType == 4)
                {
                    ...
                    var detailIndex = EditorGUILayout.Popup("Обработка строки", detailList.IndexOf(body.spotDetail), SpotDetail_s);
                ...
                EditorGUILayout.PropertyField(prop, new GUIContent("Обновлять каждый кадр"));
            }
