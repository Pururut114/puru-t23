    ...
    internal class T23_ConnectFromUdonEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (body.target)
            {
                ...
                {
                    EditorGUILayout.HelpBox("CustomTrigger отсутствует или поле CustomTrigger Name не заполнено.", MessageType.Warning);
                }
            }
            if (customNameList.Count > 0)
            {
                var index = EditorGUILayout.Popup("Имя CustomTrigger", customNameList.IndexOf(body.customTriggerName), customNameList.ToArray());
